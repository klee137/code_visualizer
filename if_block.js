...
/*	var variableStorage ={};
	...
	(first in variableStorage)  true

	eval(variableStorage["x"]+6); */




	for(int a=0; a<array_lines.length; a++){
		for(int b=0; b<array_lines[a].length; b++){
	...
	for(int a=0; a<start_index.length; a++){
		temp1=1; //starts at 1 because if line contains open bracket
		for(int b=start_index[a]+1; start_index.length; b++){
			for(int c=0; c<array_lines[b].length; c++){
				if(array_lines[b][c]==="{"){
					temp1++;
	...
	for(int a=0; a<start_index.length; a++){
		...
		for(int b=2; array_lines[temp1][b]!==")"; b++){ //b=2 because first two elements are "if" and "("
			if(isNaN(array_lines[temp1][b])){
				eval(array_lines[temp1][b] + "=" + vstorage[array_lines[temp1][b]].toString());
			}
